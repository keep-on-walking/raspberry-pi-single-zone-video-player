...
class VideoPlayer:
    ...
    def _build_command(self, source):
        ...
        cmd = [
            ...
            # X11 video output (xv for Pi 5 hardware decoding compatibility)
            '--vo=xv',
            ...
            # Network settings for RTSP streams
            '--network-timeout=10',
            '--rtsp-transport=tcp',
            '--stream-lavf-o=rtsp_flags=prefer_tcp',
            '--stream-lavf-o=timeout=5000000',
            
            # Reconnection on stream failure
            '--loop-file=inf',  # Keep trying to play the stream
            '--reconnect-streamed=yes',  # Auto-reconnect on network errors
            '--reconnect-delay-max=10',  # Max 10 seconds between reconnect attempts
