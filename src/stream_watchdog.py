...
def is_mpv_running():
    """Check if the persistent MPV instance is running.
    (The player now runs one long-lived mpv with sources loaded over IPC,
    so the source URL no longer appears in the process command line —
    match on the IPC socket argument instead.)"""
    try:
        result = subprocess.run(['pgrep', '-f', 'mpv.*--input-ipc-server=/tmp/mpvsocket-player'],
