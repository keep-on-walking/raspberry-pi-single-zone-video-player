...
class PresetManager:
            ...
            try:
                with open(self.presets_file, 'r') as f:
                    ...
                    if isinstance(data, dict) and '_default' in data:
                        self.default_preset = data.get('_default')
                        self.presets = {k: v for k, v in data.items() if k != '_default'}
                    else:
                        self.presets = data
                        self.default_preset = None
                print(f"✅ Loaded {len(self.presets)} presets")
                if self.default_preset:
                    print(f"📌 Default preset: {self.default_preset}")
    ...
    def save_preset(self, name, geometry, description=""):
        ...
        try:
            ...
            with open(self.presets_file, 'w') as f:
                data = dict(self.presets)
                if self.default_preset:
                    data['_default'] = self.default_preset
    ...
    def set_default(self, name):
        ...
        if name in self.presets or name is None:
            ...
            try:
                ...
                with open(self.presets_file, 'w') as f:
                    ...
                    if self.default_preset:
                        data['_default'] = self.default_preset
                    json.dump(data, f, indent=2)
                print(f"✅ Set default preset: {name}")
                return True
            except Exception as e:
    ...
    def get_default(self):
        """Get the default preset geometry, or None if no default set"""
        if self.default_preset and self.default_preset in self.presets:
            return self.presets[self.default_preset]['geometry']
        return None
