...
# Create empty remotes config if it doesn't exist
if [ ! -f $INSTALL_DIR/config/remotes.json ]; then
    echo "[]" > $INSTALL_DIR/config/remotes.json
    chown $ACTUAL_USER:$ACTUAL_USER $INSTALL_DIR/config/remotes.json
    echo "Created empty remotes config"
fi
